...
def generate_graph(df):
    ...
    dprime_graph.set_xticklabels([" ".join(t.get_text().split("-")[1:]).title() for t in dprime_graph.get_xticklabels()], fontsize=14)
   ...
   # dprime_graph.set(ylim=(0, 4))
    ...
    rt_graph.set_xticklabels([" ".join(t.get_text().split("-")[1:]).title() for t in rt_graph.get_xticklabels()], fontsize=14)
    rt_graph.set(ylim=(0, 1200))
    rt_graph.set_title('B', y=1, x=0.02, fontsize = 16)

    #fig.tight_layout()
    fig.figure.savefig('f1.jpg',dpi=600)
